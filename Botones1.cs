...
public class Botones1 : MonoBehaviour
{
    public GUITexture P1; 
    ...
    void Update()
    {
        ...
        coche1.transform.Translate(-Vx, 0, 0);
        ...
        if (Input.touchCount > 0)
        {
            if (P1.HitTest(Input.GetTouch(0).position))//Solo entra si es touch es  con un dedo y le das al boton
            {
                ...
                if (Input.GetTouch(0).phase == TouchPhase.Stationary)//mientras el dedo este pulsando
                {
                    ...
                    if (giroD)
                    {
                        coche1.transform.Rotate(0, 0, 10);
                    }
            ...
            if (P1.HitTest(Input.GetTouch(1).position))//Solo entra si es touch es  con un dedo y le das al boton
            {
